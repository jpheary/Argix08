...
namespace Argix {
	//
	public class PCSFileSvc : System.ServiceProcess.ServiceBase {
		//Members
		...
		public PCSFileSvc() {
            ...
            catch(Exception ex) { reportError(new ApplicationException("Unexpected error while creating new PCSFileSvc instance.",ex), false,true); }
        }
        ...
        private void reportError(Exception ex,bool displayMessage,bool logMessage) {
            ...
            try {
                ...
                if(logMessage)
                    EventLog.WriteEntry("PCSFileSvc",ex.ToString(),EventLogEntryType.Error);
            }
            catch(Exception _ex) { EventLog.WriteEntry("PCSFileSvc",_ex.ToString(),EventLogEntryType.Error);  }
        }
        #endregion
    }
}
