...
namespace Tsort.Utility {
	...
	public class frmMain : System.Windows.Forms.Form {
		//Members
        private LindaSvc mService = null;
        private NotifyIcon mIcon=null;		//Task tray icon when using internal scheduling
		
		private const int CTX_OPEN = 0, CTX_PAUSE = 1, CTX_CONTINUE = 2, CTX_STOP = 3, CTX_SEP1 = 4, CTX_LOG = 5;
		private System.ComponentModel.Container components = null;
		
		//Interface
        public frmMain(LindaSvc fileSvc) {
			//Constructor
			try {
				//Required for designer support
				InitializeComponent();
                this.mService = fileSvc;
			}
        ...
        private NotifyIcon newTrayIcon(string title,Icon icon) {
			...
			try {
				...
				ctxLog.Enabled = true;
                trayIcon.ContextMenu = new ContextMenu(new MenuItem[] { ctxOpen,ctxPause,ctxContinue,ctxStop,ctxSep1,ctxLog });
        ...
        private void setServices() {
			...
			try {
				if(this.mIcon != null) {
					this.mIcon.ContextMenu.MenuItems[CTX_OPEN].Enabled = false;
                    this.mIcon.ContextMenu.MenuItems[CTX_PAUSE].Enabled = false; // this.mTimer.Enabled;
                    this.mIcon.ContextMenu.MenuItems[CTX_CONTINUE].Enabled = false; // !this.mTimer.Enabled;
                    this.mIcon.ContextMenu.MenuItems[CTX_STOP].Enabled = true;
                    this.mIcon.ContextMenu.MenuItems[CTX_LOG].Enabled = true;
				}
			}
            catch(Exception ex) { reportError(ex,false,true); }
        }
