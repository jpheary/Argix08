...
namespace Tsort05.Utility {
	...
	public class frmMain : System.Windows.Forms.Form {
		//Members
        private DDUFileSvc mService = null;
		...
		//Interface
        public frmMain(DDUFileSvc dduFileSvc) {
			//Constructor
			try {
				//Required for designer support
				InitializeComponent();
                this.mService = dduFileSvc;
			}
		...
		private void OnFormLoad(object sender, System.EventArgs e) {
			...
			try {
				//Hide
				this.Visible = false;
                SvcLog.LogMessage("UTILTIY STARTED: user=" + Environment.UserName);
                this.mService.Start();
                if(this.mService.OneShot) this.Close();
            }
