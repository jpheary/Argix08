...
<script runat="server">
    ...
    void Trace(string message,Argix.Freight.LogLevel level) {
        //Trace
        Argix.Freight.TraceMessage m = new Argix.Freight.TraceMessage();
        m.Name = "Argix08";
        m.Source = "Argix08.TLViewer";
        m.User = HttpContext.Current.User != null ? HttpContext.Current.User.Identity.Name : "";
        m.Computer = Environment.MachineName;
        m.LogLevel = level;
        m.Message = message;
        Argix.Freight.FreightProxy fp = new Argix.Freight.FreightProxy();
        fp.WriteLogEntry(m);
    }
</script>
