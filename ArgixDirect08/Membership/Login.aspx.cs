...
public partial class Login : System.Web.UI.Page {
            ...
            if(Page.IsValid) {
                ...
                if(Membership.ValidateUser(username,this.txtPassword.Text)) {
                    ...
                    if(isPasswordExpired(username))
                        Response.Redirect("~/Members/ChangePassword.aspx",false);
                    else {
    ...
    public bool isPasswordExpired(string username) {
        ...
        int expireDays = Convert.ToInt32(WebConfigurationManager.AppSettings["PasswordExpiration"]);
        TimeSpan ts = DateTime.Today.Subtract(user.LastPasswordChangedDate);
        return (ts.Days > expireDays || profile.PasswordReset);
    }
}
