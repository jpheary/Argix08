...
namespace Argix {
    ...
    public class App :AppBase {
		...
		static App() { 
			...
			try {
                ...
                _Mediator = (Mediator)new SQLMediator(global::Argix.Properties.ArgixSettings.Default.SQLConnection);
	...
	internal class Config :AppConfig {
		//Members
		public const string KEY_DATEDAYSBACK = "DateDaysBack";
        public const string KEY_CANEDITEXPORT = "CanEditExport";
		
		//Interface
		public Config(string PCName,DataSet ds,Mediator mediator) : base(PCName,ds,mediator) { }

		[Category("Accessibility"),Description("Number of days back for trip selection.")]
		public int DateDaysBack {
			get { return GetValueAsInteger(KEY_DATEDAYSBACK); }
			set { SetValue(KEY_DATEDAYSBACK,value.ToString()); }
		}
        [Category("Behavior"), Description("Allow edit of client export configurations.")]
        public bool CanEditExport {
            get { return GetValueAsBoolean(KEY_CANEDITEXPORT); }
            set { SetValue(KEY_CANEDITEXPORT, value.ToString()); }
        }
    }
