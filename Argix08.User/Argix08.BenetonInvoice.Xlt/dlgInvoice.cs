...
namespace Argix.Finance {
    ...
    public partial class dlgInvoice :Form {
        ...
        private const string USP_INVOICE = "uspInvBenettonInvoiceGetList",TBL_INVOICE = "ClientInvoiceTable";
        ...
        public DateTime InvoiceDate { get { return Convert.ToDateTime(this.grdInvoice.SelectedRows[0].Cells["invoiceDateDataGridViewTextBoxColumn"].Value.ToString()); } }
        private void OnFormLoad(object sender,EventArgs e) {
            ...
            try {
                ...
                adapter.Fill(this.mInvoiceDS,TBL_CLIENT);
                string clientID = global::Argix.Finance.Settings.Default.ClientID;
                if(this.cboClient.Items.Count > 0) {
                    if(clientID.Length > 0) this.cboClient.SelectedValue = clientID;
                    else this.cboClient.SelectedIndex = 0;
                }
                OnClientSelected(null,EventArgs.Empty);
                this.cboClient.Enabled = (global::Argix.Finance.Settings.Default.ClientID.Length == 0);
            }
