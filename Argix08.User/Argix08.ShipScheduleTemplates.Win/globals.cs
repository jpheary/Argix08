...
namespace Argix {
    ...
    public class App :AppBase {
        //Members
        private static ShipScheduleConfiguration _Config = null;

        //Interface
        static App() {
            //Class constructor: get application configuration
            try {
                _Config = new ShipScheduleConfiguration();
            }
            catch (Exception ex) { ReportError(ex); Application.Exit(); }
        }
        private App() { }
        public static ShipScheduleConfiguration Config { get { return _Config; } }
        ...
        public static void Trace(string message, LogLevel level) {
            ...
            if (level >= _Config.TraceLevel) {
                ...
                m.LogLevel = level;
                m.Message = message;
                ShipScheduleProxy.WriteLogEntry(m);
            }
        }
    ...
    public class ShipScheduleConfiguration {
        //Memnbers
        private UserConfiguration mConfig = null;
        
        public ShipScheduleConfiguration() {
            //Constructor
            this.mConfig = ShipScheduleProxy.GetUserConfiguration(App.Product, new string[] { Environment.UserName, Environment.MachineName });
        }
        ...
        [Category("Security"), Description("User role- grants application access rights (i.e. LineHaulOperator, LineHaulCoordinator, LineHaulAdministrator).")]
        public string Role { get { return this.mConfig["Role"]; } set { this.mConfig["Role"] = value; } }
    }

    public class AppSecurity {
        //Members
        public static string Role = ROLE_NONE;
        public static string ROLE_NONE = "LineHaulOperator", ROLE_COORDINATOR = "LineHaulCoordinator", ROLE_ADMINISTRATOR = "LineHaulAdministrator";

        //Interface
        static AppSecurity() { }
        private AppSecurity() { }
        public static bool UserCanAccessTemplate { get { return AppSecurity.Role.ToLower() == ROLE_ADMINISTRATOR.ToLower(); } }
    }
}