...
//	Author:	J. Heary
//	Date:	04/28/06
//	Desc:	Dialog to create a new or edit an existing (agent) terminal location.
//	Rev:	
//	---------------------------------------------------------------------------
using System;
...
namespace Tsort {
	//
	public class dlgTerminalDetail : System.Windows.Forms.Form {
		//Members
		...
		private Tsort.Enterprise.TerminalDS mTerminalDS;
		...
		//Interface
		public dlgTerminalDetail(bool parentIsActive, ref TerminalDS terminal) {
			//Constructor
			try {
				...
				if(this.mTerminalDS.TerminalDetailTable.Count > 0) {
					this.mTerminalID = this.mTerminalDS.TerminalDetailTable[0].LocationID;
					this.Text = (this.mTerminalID>0) ? "Terminal (" + this.mTerminalID + ")" : "Terminal (New)";
				}
				else
					this.Text = "Terminal Location (Data Unavailable)";
			} 
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(dlgTerminalDetail));
			...
			this.mTerminalDS = new Tsort.Enterprise.TerminalDS();
			...
			this.mTerminalDS.Locale = new System.Globalization.CultureInfo("en-US");
			// 
			// fraContact
			// 
		...
		private void OnFormLoad(object sender, System.EventArgs e) {
			...
			try {
				...
				this.txtNumber.Text = this.mTerminalDS.TerminalDetailTable[0].Number;
				this.txtNumber.Enabled = (mTerminalID==0);
				this.txtName.MaxLength = 40;
				this.txtName.Text = this.mTerminalDS.TerminalDetailTable[0].Description;
				this.txtMnemonic.MaxLength = 3;
				this.txtMnemonic.Text = "";
				if(!this.mTerminalDS.TerminalDetailTable[0].IsMnemonicNull())
					this.txtMnemonic.Text = this.mTerminalDS.TerminalDetailTable[0].Mnemonic;
				
				this.txtLine1.MaxLength = 40;
				this.txtLine1.Text = this.mTerminalDS.AddressDetailTable[0].AddressLine1;
				this.txtLine2.MaxLength = 40;
				if(!this.mTerminalDS.AddressDetailTable[0].IsAddressLine2Null())
					this.txtLine2.Text = this.mTerminalDS.AddressDetailTable[0].AddressLine2;
				this.txtCity.MaxLength = 40;
				this.txtCity.Text = this.mTerminalDS.AddressDetailTable[0].City;
				//if(this.mTerminalDS.TerminalDetailTable[0].IsStateOrProvinceNull()) 
				//	this.cboStates.SelectedIndex = 0;
				//else
				this.cboStates.SelectedValue = this.mTerminalDS.AddressDetailTable[0].StateOrProvince;
				this.cboStates.Enabled = (this.cboStates.Items.Count>0);
				this.txtZip.MaxLength = 15;
				//if(!this.mTerminalDS.TerminalDetailTable[0].IsPostalCodeNull())
					this.txtZip.Text = this.mTerminalDS.AddressDetailTable[0].PostalCode;
				if(this.mTerminalDS.AddressDetailTable[0].CountryID==0) 
					this.cboCountries.SelectedIndex = 0;
				else
					this.cboCountries.SelectedValue = this.mTerminalDS.AddressDetailTable[0].CountryID;
				this.cboCountries.Enabled = (this.cboCountries.Items.Count>0);
				
				this.txtContact.MaxLength = 30;
				if(!this.mTerminalDS.TerminalDetailTable[0].IsContactNameNull())
					this.txtContact.Text = this.mTerminalDS.TerminalDetailTable[0].ContactName;
				this.mskPhone.InputMask = "###-###-####";
				if(!this.mTerminalDS.TerminalDetailTable[0].IsPhoneNull())
					this.mskPhone.Value = this.mTerminalDS.TerminalDetailTable[0].Phone;
				this.txtExt.MaxLength = 4;
				if(!this.mTerminalDS.TerminalDetailTable[0].IsExtensionNull())
					this.txtExt.Text = this.mTerminalDS.TerminalDetailTable[0].Extension;
				this.mskFax.InputMask = "###-###-####";
				if(!this.mTerminalDS.TerminalDetailTable[0].IsFaxNull())
					this.mskFax.Value = this.mTerminalDS.TerminalDetailTable[0].Fax;
				if(!this.mTerminalDS.TerminalDetailTable[0].IsEmailNull())
					this.txteMail.Text = this.mTerminalDS.TerminalDetailTable[0].Email;
				
				this.chkStatus.Checked = this.mTerminalDS.TerminalDetailTable[0].IsActive;
				if(!mParentIsActive) {
					//If parent is inactive: 1. Status MUST be inactive for new
					//					     2. Status cannot be changed for new or existing
					if(mTerminalID==0) this.chkStatus.Checked = false;
					this.chkStatus.Enabled = false;
				}
				
				//Transporatation
				if(!this.mTerminalDS.TerminalDetailTable[0].IsOpenTimeNull())
					this.dtpOpen.Value = this.mTerminalDS.TerminalDetailTable[0].OpenTime;
				if(!this.mTerminalDS.TerminalDetailTable[0].IsCloseTimeNull())
					this.dtpClose.Value = this.mTerminalDS.TerminalDetailTable[0].CloseTime;

				//Freight
				this.txtSAN.MaxLength = 7;
				this.txtSAN.Text = "";
				this.txtSAN.Enabled = false;
//				if(!this.mTerminalDS.TerminalDetailTable[0].IsSanNumberNull())
//					this.txtSAN.Text = this.mTerminalDS.TerminalDetailTable[0].SanNumber;
				this.txtLabelData.MaxLength = 25;
				this.txtLabelData.Text = "";
				if(!this.mTerminalDS.TerminalDetailTable[0].IsUserLabelDataNull())
					this.txtLabelData.Text = this.mTerminalDS.TerminalDetailTable[0].UserLabelData;
				if(!this.mTerminalDS.TerminalDetailTable[0].IsExceptionNull()) {
					this.chkIsLocationException.Checked = (false && this.mTerminalDS.TerminalDetailTable[0].Exception=="");
					this.chkIsLocationException.Checked = (false && this.mTerminalDS.TerminalDetailTable[0].Exception!="");
				}
				else
					OnLocationExceptionChecked(null, null);
				this.chkIsLocationException.Enabled = false;
			}
		...
		private void OnLocationExceptionChecked(object sender, System.EventArgs e) {
			...
			try {
				if(this.chkIsLocationException.Checked == true) {
					...
					this.txtLocationException.Text = "";
					if(!this.mTerminalDS.TerminalDetailTable[0].IsExceptionNull())
						this.txtLocationException.Text = this.mTerminalDS.TerminalDetailTable[0].Exception;
					this.chkArgix.Checked = false;
					if(!this.mTerminalDS.TerminalDetailTable[0].IsIsArgixUseNull())
						this.chkArgix.Checked = this.mTerminalDS.TerminalDetailTable[0].IsArgixUse;
				}
				else {
					//Clear fields
		...
		private void OnCmdClick(object sender, System.EventArgs e) {
			...
			try {
				...
				switch(btn.Text) {
					...
					case CMD_OK:
						//General
						this.mTerminalDS.TerminalDetailTable[0].Number = this.txtNumber.Text;
						this.mTerminalDS.TerminalDetailTable[0].Description = this.txtName.Text;
						if(this.txtMnemonic.Text!="")
							this.mTerminalDS.TerminalDetailTable[0].Mnemonic = this.txtMnemonic.Text;
						else
							this.mTerminalDS.TerminalDetailTable[0].SetMnemonicNull();						
						this.mTerminalDS.AddressDetailTable[0].AddressLine1 = this.txtLine1.Text;
						if(this.txtLine2.Text!="")
							this.mTerminalDS.AddressDetailTable[0].AddressLine2 = this.txtLine2.Text;
						else
							this.mTerminalDS.AddressDetailTable[0].SetAddressLine2Null();
						this.mTerminalDS.AddressDetailTable[0].City = this.txtCity.Text;
						this.mTerminalDS.AddressDetailTable[0].StateOrProvince = this.cboStates.SelectedValue.ToString();
						this.mTerminalDS.AddressDetailTable[0].PostalCode = this.txtZip.Text;
						this.mTerminalDS.AddressDetailTable[0].CountryID = (int)this.cboCountries.SelectedValue;
						
						if(this.txtContact.Text!="")
							this.mTerminalDS.TerminalDetailTable[0].ContactName = this.txtContact.Text;
						else
							this.mTerminalDS.TerminalDetailTable[0].SetContactNameNull();
						this.mTerminalDS.TerminalDetailTable[0].Phone = this.mskPhone.Value.ToString();
						if(this.txtExt.Text!="")
							this.mTerminalDS.TerminalDetailTable[0].Extension = this.txtExt.Text;
						else
							this.mTerminalDS.TerminalDetailTable[0].SetExtensionNull();
						if(this.mskFax.Value!=System.DBNull.Value)
							this.mTerminalDS.TerminalDetailTable[0].Fax = this.mskFax.Value.ToString();
						else
							this.mTerminalDS.TerminalDetailTable[0].SetFaxNull();
						if(this.txteMail.Text!="")
							this.mTerminalDS.TerminalDetailTable[0].Email = this.txteMail.Text;
						else
							this.mTerminalDS.TerminalDetailTable[0].SetEmailNull();
						this.mTerminalDS.TerminalDetailTable[0].IsActive = this.chkStatus.Checked;
						
						//Transportation
						if(this.fraHours.Enabled) {
							this.mTerminalDS.TerminalDetailTable[0].OpenTime = this.dtpOpen.Value;
							this.mTerminalDS.TerminalDetailTable[0].CloseTime = this.dtpClose.Value;
						}
						else {
							this.mTerminalDS.TerminalDetailTable[0].SetOpenTimeNull();
							this.mTerminalDS.TerminalDetailTable[0].SetCloseTimeNull();
						}
						
						//Freight
//						if(this.txtSAN.Text!="")
//							this.mTerminalDS.TerminalDetailTable.SanNumber = this.txtSAN.Text;
//						else
//							this.mTerminalDS.TerminalDetailTable.SetSanNumberNull();
						if(this.txtLabelData.Text!="")
							this.mTerminalDS.TerminalDetailTable[0].UserLabelData = this.txtLabelData.Text;					
						else
							this.mTerminalDS.TerminalDetailTable[0].SetUserLabelDataNull();
						if(this.chkIsLocationException.Checked) {
							this.mTerminalDS.TerminalDetailTable[0].Exception = this.txtLocationException.Text;
							this.mTerminalDS.TerminalDetailTable[0].IsArgixUse = this.chkArgix.Checked;
						}
						else {
							this.mTerminalDS.TerminalDetailTable[0].SetExceptionNull();
							this.mTerminalDS.TerminalDetailTable[0].SetIsArgixUseNull();
						}
