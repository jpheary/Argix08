	...
	public class ShipSchedule {
		...
		public void Refresh() {
			...
			try {
				...
				DataSet ds = App.Mediator.FillDataset(USP_SCHEDULE, TBL_SCHEDULE, new object[]{this.mSortCenterID, this.mScheduleDate});
				if(ds.Tables[TBL_SCHEDULE].Rows.Count > 0) 
					this.mTrips.Merge(ds, true, MissingSchemaAction.Ignore);

                ds = App.Mediator.FillDataset(USP_TEMPLATES_AVAILABLE,TBL_TEMPLATES_AVAILABLE,new object[] { this.mSortCenterID,this.mScheduleDate });
                if(ds.Tables[TBL_TEMPLATES_AVAILABLE].Rows.Count > 0) { 
		...
		public void Create() {
			...
			try {
				...
				this.mScheduleID = (string)App.Mediator.ExecuteNonQueryWithReturn(USP_SCHEDULENEW, new object[]{ this.mScheduleID, this.mSortCenterID, this.mScheduleDate, DateTime.Now, Environment.UserName });
				for(int i=0; i<this.mTemplates.TemplateViewTable.Rows.Count; i++) {
					...
					if(this.mTemplates.TemplateViewTable[i].IsMandatory == 1)
						App.Mediator.ExecuteNonQuery(USP_TRIPNEW, new object[]{ "", this.mScheduleID, this.mTemplates.TemplateViewTable[i].TemplateID, DateTime.Now, Environment.UserName });
				}
				Refresh();
			}
			catch(ApplicationException ex) { throw ex; }
				...
				if(trips != null && trips.ShipScheduleTable.Rows.Count > 0) {
					...
					foreach(ShipScheduleDS.ShipScheduleTableRow row in trips.ShipScheduleTable.Rows) {
						...
						if(isLoadNumberOrCarrierChanged(row)) {
							DataSet loadNumberDS = App.Mediator.FillDataset(USP_TRIP, TBL_TRIP, new object[]{row.ScheduleDate, System.DBNull.Value, row.LoadNumber.Trim()});
			...
			try {
				...
				using(SqlTransaction sqlTrans = sqlConn.BeginTransaction()) {
					try {
						...
						SqlHelper.ExecuteNonQuery(sqlTrans, CommandType.StoredProcedure, USP_TRIPUPDATE, spTripParams);
					}
					...
					try {
						...
						SqlHelper.ExecuteNonQuery(sqlTrans, CommandType.StoredProcedure, USP_STOPUPDATE, spStop1Params);
					}
					...
					try {
						...
						if(trip.ShipScheduleStopTable.Rows.Count == 2) {
							...
							SqlHelper.ExecuteNonQuery(sqlTrans, CommandType.StoredProcedure, USP_STOPUPDATE, spStop2Params);
						}
					}
